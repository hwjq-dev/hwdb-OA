...
import { OrganizationalStructureCard } from './organizational-structure-card';
...
export const OrganizationStructureList = () => {
    ...
    <div className={cn('flex flex-col space-y-8 mt-4')}>
      {structuredData.map((x, i) => (
        ...
        >
          ...
          {(x.data || []).map((x, j) => (
            <Link href={ROUTES.$ORGANZATIONAL_STRUCTURE(x.id)} key={j}>
              <OrganizationalStructureCard {...x} />
            </Link>
          ))}
        </div>
      ))}
