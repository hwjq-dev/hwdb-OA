    ...
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: cn(
          ...
          'h-7 w-7 bg-transparent p-0 hover:opacity-100 absolute left-5 top-5',
          disableLeftNavigation() && 'pointer-events-none',
        ),
        button_next: cn(
          buttonVariants({ variant: 'outline' }),
          'h-7 w-7 bg-transparent p-0 hover:opacity-100 absolute right-5 top-5',
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="flex -mt-3 gap-2">
              ...
              >
                <SelectTrigger className="focus:bg-background bg-black/5 shadow-none focus:text-accent-foreground w-fit gap-1 text-[13px] border-0 font-semibold">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
              ...
              >
                <SelectTrigger className="focus:bg-background bg-black/5 shadow-none focus:text-accent-foreground w-fit gap-1 font-semibold border-0 text-[13px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {YEARS.map((year) => (
...
const DateTimePicker = forwardRef<Partial<DateTimePickerRef>, DateTimePickerProps>(
  (
    {
      locale = enUS,
    ...
    return (
      <Popover>
        ...
        <PopoverContent className="w-auto p-0">
          ...
          />
          {granularity !== 'day' && (
            <div className="bg-gray-100 border-t-[1px] p-3">
              <TimePicker
                onChange={(value) => {
                  onChange?.(value);
                  setDisplayDate(value);
